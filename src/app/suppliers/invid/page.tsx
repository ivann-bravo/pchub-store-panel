...
export default function InvidSupplierPage() {
  const router = useRouter();
  const { data: session, status } = useSession();
  const [info, setInfo] = useState<InvidInfo | null>(null);
  const [loading, setLoading] = useState(true);
  const [importing, setImporting] = useState(false);
  const [savingConfig, setSavingConfig] = useState(false);
  const [lastResult, setLastResult] = useState<ImportResult | null>(null);
  const [lowStockQty, setLowStockQty] = useState(5);
  const [defaultStockQty, setDefaultStockQty] = useState(20);

  const fetchInfo = useCallback(async () => {
    try {
      const res = await fetch("/api/suppliers/invid/import");
      const data = await res.json();
      setInfo(data);
      if (data.supplier?.stockConfig) {
        try {
          const cfg = JSON.parse(data.supplier.stockConfig);
          setLowStockQty(cfg.lowStockQty ?? 5);
          setDefaultStockQty(cfg.defaultStockQty ?? 20);
        } catch {}
      }
    } catch {
      toast.error("Error al cargar información de Invid");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      const res = await fetch(`/api/suppliers/${info.supplier.id}`, {
        ...
        body: JSON.stringify({
          stockConfig: JSON.stringify({ lowStockQty, defaultStockQty }),
        }),
      });
      if (res.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold">Invid — Importar Catálogo</h1>
          <p className="text-sm text-muted-foreground">
            Formato XLSX · Detección automática de columnas
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Subí la lista de precios XLSX de Invid. Las columnas se detectan automáticamente.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleImport} className="space-y-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Columnas leídas: Codigo · Producto · Fabricante · Nro. de Parte · Precio sin IVA · %IVA · Imp. Int. · Observaciones
                </p>
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Invid usa &quot;Observaciones&quot; para indicar stock bajo. Configurá las cantidades que se asignan.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-end gap-6">
            <div className="space-y-1.5">
              <Label htmlFor="low-stock-qty">Cantidad &quot;Stock Bajo&quot;</Label>
              <Input
                id="low-stock-qty"
                type="number"
                min={0}
                ...
                className="w-32"
              />
              <p className="text-xs text-muted-foreground">Cuando dice &quot;Stock Bajo&quot;</p>
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="default-stock-qty">Cantidad normal</Label>
              <Input
                ...
                className="w-32"
              />
              <p className="text-xs text-muted-foreground">Sin observación especial</p>
            </div>
            <Button
